...
## Custom `Dataset` - CSES-3419 (https://cses.fi/problemset/task/3419) (nimber calculation)
...
## Higher-level `DataLoader` with custom `DataSource` - CSES-1621 (https://cses.fi/problemset/task/1621) (size(unique(...)))
...
## `DataSource` as a library - CSES-1619 (https://cses.fi/problemset/task/1619) (interval overlapping)

Make the DataSource a [separate library](../src/my_lib/tests_data_source.py) for reuse.

[Whole program](cses-1619/whole.py).

## Add transformation - CSES-1640 (https://cses.fi/problemset/task/1640) (two values from array)
...
## Add input mask - CSES-1643 (https://cses.fi/problemset/task/1643) (prefix max)

Formalize data padding a little: explicit input mask (boolean array) to tell which elements are original inputs and which are padded. Need this before doing batching.
