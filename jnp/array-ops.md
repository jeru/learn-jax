...
Eg., one can do `block([[A, B], [C, D]])` each one of `A, B, C, D` is a matrix/array itself.
...
`eye` allows non-square and the ones to be offset from the diagnal by `k`.
...
: similar to `diag`'s 1D->2D version, but `v` can be multi-dimensional and will be flattened into 1D. (Why does this need to exist? Just ask the user to call two functions nestedly.)
...
Transpose and its generalization can also fit in this bucket, but is instead in section [Shape manipulation](shape-manipulation.md).
...
: returns an array (dynamic-sized if `size` is not given) of indices of non-zero elements in `a`, each index is n dimensions if `a` is an n-order tensor/array. DISCOURAGE: prefer `nonzero` for consistent index array format with the remaining of system. `argwhere`'s return shape is an outlier.
